...
<body>
	...
	<script>
		$(function() {
			...
			$("#ajax_btn").click(function() {
				...
				$.ajax({
					type : "GET",
					url : "http://localhost:8080/SpringMVCproject/案件getTestData",
			...
			$("#社員参照_btn").click(function() {
				executeAction('http://localhost:8080/SpringMVCproject/社員getTestData');
			});
			...
			$.ajax({
				...
				success : function(data) {
					...
					if(data[0]){
						...
						//---------------------------------
						$("#theForm").attr("action",
								"http://localhost:8080/SpringMVCproject/案件edit");
						$("#theForm").submit();
						//---------------------------------
					}else{
						alert("没有找到检索对象。");
		...
		function oneRowDeleteClick(selectedID) {
			...
			$.ajax({
				type : 'POST',
				url : "http://localhost:8080/SpringMVCproject/案件delete",
				dataType : "json", //dataType设置成 json，这个意思是说 ’服务器的数据返回的是json格式数据，需要帮我把数据转换成对象
				contentType : "application/json",
				...
				success : function(data) {
					...
					$("#theForm").attr("action",
							"http://localhost:8080/SpringMVCproject/案件getTestData");
					$("#theForm").submit();
				},
				error : function(e) {
	...
	<form name="theForm" id="theForm" method="get" action="http://localhost:8080/SpringMVCproject/add案件">
		<h1>技術者情報</h1>
		<br>
		<div>
